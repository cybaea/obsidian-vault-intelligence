...
3.  **Boosting**: If a document appears in *both* result sets, we apply a massive boost: `Score = VectorScore * (1 + KeywordScore)`.

## Consequences

### Positive
*   **"Best of Both Worlds"**: The user gets the precision of `grep` with the intelligence of LLMs.
*   **Trust**: Users trust the tool more when it reliably finds the file they *know* exists.
