...
-   Fix unnecessary re-embedding of previously-indexed notes on every plugin restart. Two issues caused stale mtime/size values to persist in the graph state: (1) the persistence save was running concurrently with index mutations via `executeQuery` instead of `executeMutation`, causing `graph.export()` to capture a partially-updated graph; (2) the final save at the end of `scanAll` used `requestIdleCallback` which may not fire before the plugin is unloaded (e.g., "Reload app without saving"). Both are now fixed — the save serializes through the mutation queue and runs directly (awaited) at scan completion.
...
-   Update all non-major dependencies ([#508](https://github.com/cybaea/obsidian-vault-intelligence/pull/508))
...
-   Update the stale types from the Orama v2 -> v3 transition, second try.
...
-   chore(deps): update all non-major dependencies [#483](https://github.com/cybaea/obsidian-vault-intelligence/pull/483)
...
-   **Updated packages**: Security enhancements by updating dependencies.
...
-   **Tool security alignment**: Updated `ToolRegistry` path validation to continue protecting `.vault-intelligence` while decoupling legacy excluded-folder behaviour from agent write tool security.
...
-   **Accurate Token Budgeting**: Implemented precise token estimation for Gardener by stringifying individual note objects to account for JSON overhead.
-   **Security Updates**:Updated packages and applied security fixes.
...
-   **Security-first execution**: The MCP integration features a built-in "Trust but Verify" modal and mathematical trust hashing. Remote code execution is strictly gated, ensuring that potentially destructive tools cannot run without your explicit approval, effectively eliminating zero-click RCE vulnerabilities.
...
-   **Gemini 3.1 Native Grounding**: The Researcher now natively utilizes `google_search` tools within the Gemini 3.1 architecture. Web search grounding and embedded citations are processed directly by the main agent rather than requiring a secondary sub-agent workflow, resulting in significantly faster and more fluid responses.
...
-   **Interactive researcher links**: Internal wikilinks (e.g. `[[Note Name]]`) generated by the AI in the Researcher chat are now fully clickable and instantly open the corresponding note in your vault. Native modifier keys (like `Ctrl`+click to open in a new tab) are also supported.
...
-   **Context management refinements**: Context window budgets are now configured per model rather than globally. Engineered a balanced context assembler that structurally guarantees single large documents cannot dominate the context window budget during assembly.
...
-   **Robust JSON Lexer parsing**: Implemented a lightweight, state-machine JSON lexer to power the `OllamaProvider`. This deeply improves the reliability of local model tool execution by flawlessly extracting partial structured data from streams, even when complex nested brackets or string escape characters (`\"`) are generated by the model.
...
-   **Enterprise-Grade MCP Security**: Designed a robust defense-in-depth architecture within `McpClientManager`. The plugin now natively leverages Obsidian's OS-encrypted `SecretStorage` for all MCP environment variables and connection headers. Furthermore, the execution environment automatically scrubs sensitive host environment variables from child processes—exposing only a strict whitelist of safe variables like `PATH` and `DISPLAY`—and enforces mandatory Trust Hash verification across all transports to structurally prevent zero-click RCE vulnerabilities.
...
-   **MCP tool output preservation**: Engineered intelligent JSON context truncation for MCP tool outputs, guaranteeing structure preservation while strictly adhering to model context limits, alongside centralized execution timeouts.
...
-   **Rabin-Karp algorithmic upgrade**: Implemented a true Rabin-Karp Modulo-Polynomial rolling hash for $O(N)$ link parsing and context hydration, completely eliminating main-thread blocking across large documents.
...
-   **Semantic Galaxy View**: Augmented the static relationship list with a high-performance, interactive 3D-like graph view. The "Semantic Galaxy" visualises your vault's relationships in real-time, centring on your active note.
    ...
    -   **Adaptive Rendering**: Edge labels dynamically scale and shift colouring to support high contrast modes like Obsidian Dark Mode natively.
