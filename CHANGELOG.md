...
-   **Model Context Protocol (MCP)**: Extend the Researcher agent with custom tools using MCP. Connect to any standard MCP server via local execution (`stdio`) or remote URLs (`sse`), unlocking capabilities like local filesystem access, database querying, or API interactions.
-   **Enterprise MCP support**: Added full support for the modern `streamable_http` MCP transport, allowing connections to advanced network servers and secure enterprise endpoints.
-   **Secure MCP authentication**: You can now provide custom HTTP headers (eg `Authorization: Bearer token`) for both `streamable_http` and `sse` remote MCP servers, enabling secure connections to private infrastructure.
-   **Dynamic settings reactivity**: Enabling, disabling, or modifying MCP servers in settings now instantly updates the Researcher's available tools without requiring a chat reset or application reload.
...
-   **Gemini 3.1 Native Grounding**: The Researcher now utilizes the native `google_search` tools within the Gemini 3.1 architecture to perform faster, higher-quality web search grounding with embedded citations directly in the response stream.
