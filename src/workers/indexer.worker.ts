...
const IndexerWorker: WorkerAPI = {
    async buildPriorityPayload(queryVector: number[], query: string): Promise<unknown[]> {
        ...
        for (const hit of vectorResults.hits) {
            const doc = (hit as unknown as OramaResultHit).orama_document;
    ...
    async findOntologySynonyms(semanticThreshold: number): Promise<SynonymCandidate[]> {
        ...
        for (const node of ontologyNodes) {
            ...
            if (attr.size > ONTOLOGY_CONSTANTS.TOPIC_MIN_EXPLICIT_BYTES) {
                ...
                if (results.hits.length > 0 && (results.hits[0] as unknown as OramaResultHit)?.orama_document?.embedding) {
                    topicEmbedding = (results.hits[0] as unknown as OramaResultHit).orama_document.embedding || [];
    ...
    async getSimilar(path: string, limit: number = WORKER_INDEXER_CONSTANTS.SEARCH_LIMIT_DEFAULT, minScore: number = 0): Promise<GraphSearchResult[]> {
        ...
        return maxPoolResults(results.hits.map(h => ({
            doc: (h as unknown as OramaResultHit).orama_document,
            id: h.id,
            score: h.score
        })), limit, minScore);
    },
    ...
    async search(query: string, limit: number = WORKER_INDEXER_CONSTANTS.SEARCH_LIMIT_DEFAULT): Promise<GraphSearchResult[]> {
        ...
        for (const hit of vectorResults.hits) {
            ...
            hits.set(h.id, { doc: h.orama_document, id: h.id, score: h.score });
        }
        ...
        for (const hit of keywordResults.hits) {
            ...
            const h: OramaHit = { doc: hTyped.orama_document, id: hTyped.id, score: hTyped.score };
